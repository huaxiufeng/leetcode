package algorithm.java;

/**
 * A rectangle is represented as a list [x1, y1, x2, y2], where (x1, y1) are the coordinates of its bottom-left corner, and (x2, y2) are the coordinates of its top-right corner.

 Two rectangles overlap if the area of their intersection is positive.  To be clear, two rectangles that only touch at the corner or edges do not overlap.

 Given two (axis-aligned) rectangles, return whether they overlap.

 * Created by huaxiufeng on 18/12/6.
 */
public class LC836RectangleOverlap {

    public boolean isRectangleOverlap(int[] rec1, int[] rec2) {
        if (null == rec1 || rec1.length != 4 || null == rec2 || rec2.length != 4) {
            return false;
        }
        return !(rec2[0] >= rec1[2] || rec2[2] <= rec1[0] || rec2[1] >= rec1[3] || rec2[3] <= rec1[1]);
    }

    public static void main(String[] args) {
        LC836RectangleOverlap solution = new LC836RectangleOverlap();
        System.out.println(solution.isRectangleOverlap(new int[]{7,8,13,15}, new int[]{10,8,12,20})); // true
        System.out.println(solution.isRectangleOverlap(new int[]{0,0,2,2}, new int[]{1,1,3,3})); // true
        System.out.println(solution.isRectangleOverlap(new int[]{0,0,1,1}, new int[]{1,0,2,1})); // false
    }
}
